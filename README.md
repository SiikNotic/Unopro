...
- **Modos de juego → Clásico / Equipos** abre la mesa de juego (`src/components/table/`). La mesa solo lee el `GameState` y envía `GameAction`s; los rivales los mueve un controlador provisional (`src/game/controllers/placeholderBot.ts`) que se sustituirá por los bots. Panel de debug solo en desarrollo.
