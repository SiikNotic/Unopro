...
| 9 | La migración de invitado a cuenta confía en el saldo que reporta el cliente (tope 25 000) | 🟡 | T | `supabase/migrations/20260927000000_profiles_staff.sql` | 352–398 | Un cliente puede declarar hasta 25 000 coins al crear su cuenta | Integridad de la economía virtual (no es dinero) | Opcional: bajar el tope o derivarlo de un saldo de invitado guardado en servidor | No | **Mitigado**: tope bajado a 5 000 (`20261001000000_guest_migration_cap.sql`, aplicado en prod) |
| 10 | Sin CAPTCHA en el registro: farming de bono de bienvenida | 🟡 | T | Supabase Auth | — | Cuentas desechables pueden acumular bonos | Integridad de la economía virtual | Activar CAPTCHA (hCaptcha/Turnstile) en Supabase Auth y confirmación por email | No | **Implementado en código** (Turnstile en registro, login, reset y sesión de invitado; workflow `supabase-auth.yml` lo activa en Supabase). Pendiente: crear el widget en Cloudflare y poner `VITE_TURNSTILE_SITE_KEY` + `TURNSTILE_SECRET_KEY` |
